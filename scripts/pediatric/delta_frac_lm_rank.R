...
# PURPOSE: Build linear models for responding stimulated delta fractions.
...
df_frac <- read_tsv(file.path(results_folder, "responding_per_subj_delta_frac_m_f.tsv")) 
...
# Figure path.
fig_path <- file.path(figures_folder, "delta_lm_rank")
# dir.create(fig_path, recursive = TRUE, showWarnings = FALSE)

fig_path_marg <- file.path(figures_folder, "delta_lm_marginal_effects_rank")
# dir.create(fig_path_marg, recursive = TRUE, showWarnings = FALSE)
...
for(i in 1:length(df_split)){
  lm_dat <- df_split[[i]] %>%
    dplyr::mutate("frac_rank" = rank(delta_frac))
  ...
  file_name <- paste0("delta_",cp,"_",st,".png")
  ...
  #   file_name <- paste0("delta_",cp,"_",st,"_marg_eff.png")
  #   ggsave(file_name, plot = p, path = fig_path_marg, 
  #          width = 14, height = 5, device = "png", dpi = 300)
}

# write_tsv(df_lm_out, file.path(results_folder, "delta_frac_lm_rank.tsv"))
