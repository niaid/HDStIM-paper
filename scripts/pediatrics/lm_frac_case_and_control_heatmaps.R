...
# PURPOSE: To generate heatmaps from linear regressions
# done intependently on case and control samples. 
...
frac_lm <- read_tsv(file.path(results_folder, "resp_unstim_frac_lm_rank_case.tsv"))
frac_age <- frac_lm %>% dplyr::filter(term == "age_at_sample") %>%
...
file_n <- file.path(figures_folder, "resp_unstim_frac_lm_case_heatmap.png")
mat <- t(mat)
...
# CONTROL
# Load fraction lm data based on ranks. 
frac_lm <- read_tsv(file.path(results_folder, "resp_unstim_frac_lm_rank_control.tsv"))
frac_age <- frac_lm %>% dplyr::filter(term == "age_at_sample") %>%
  dplyr::mutate("p.value.log" = -log10(p.value))
...
file_n <- file.path(figures_folder, "resp_unstim_frac_lm_control_heatmap.png")
mat <- t(mat)
col_fun = colorRamp2(c(0, 1, 3), c("blue", "white", "red"))
col_fun(seq(0, 3))
png(filename = file_n, width = 5, height = 6, units = "in", res = 300)
