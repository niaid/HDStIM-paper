...
yaml_file = "pregnancy_baseline.yaml"
...
ggsave("figure1.pdf", plot = fig1, device = "pdf", path = figures_folder, 
...
n_cells(sce)
fig2 <- plotCounts(sce, group_by = "sample_id", color_by = "condition")
ggsave("figure2.pdf", plot = fig2, device = "pdf", path = figures_folder, 
...
fig3 <- pbMDS(sce, color_by = "condition")
ggsave("figure3.pdf", plot = fig3, device = "pdf", path = figures_folder, 
...
# functional markers.
pdf(file =  file.path(figures_folder, "figure4.pdf"), width = 16, height = 18)
